...
impl<'a> ClientServerMsg<'a> {
    pub fn dequeue_and_decode(
        input_buffer: &[u8],
        sender: u16,
    ) -> Option<(usize, anyhow::Result<ClientServerMsg>)> {
    ...
    pub fn decode(input_buffer: &[u8], sender: u16) -> anyhow::Result<ClientServerMsg> {
        ...
        let msg = match msg_type_index {
            ...
            4 => {
                ...
                let client_type =
                    ClientType::from_u32(client_type_index).context("unsupported client id")?;
    ...
    pub fn pack(&self, wtr: &mut impl Write) {
        match self {
            ...
            ClientServerMsg::BinaryMessageTo(address, bytes) => match address {
                ...
                Address::All => {
                    wtr.write_u32::<LittleEndian>(4 + bytes.len() as u32)
                        .unwrap();
