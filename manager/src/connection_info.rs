...
fn detect_server_ip() -> Result<String, Box<dyn std::error::Error>> {
    ...
    if let Ok(output) = Command::new("sh")
        .args([
            "-c",
            "ip route get 1 2>/dev/null | awk '{print $NF;exit}' 2>/dev/null || echo ''",
        ])
        .output()
    {
        let ip = String::from_utf8_lossy(&output.stdout).trim().to_string();
...
pub fn print_connection_info(config: &ConnectionConfig, local_ip: IpAddr, public_ip: &str) {
    ...
    println!(
        "  • HTTP:  http://127.0.0.1:{}  (local machine)",
        config.http_port
    );
    println!(
        "  • HTTP:  http://{}:{}  (local network)",
        local_ip, config.http_port
    );

    if config.has_certificates {
        println!(
            "  • HTTPS: https://127.0.0.1:{}  (self-signed cert)",
            config.https_port
        );
        println!(
            "  • HTTPS: https://{}:{}  (self-signed cert)",
            local_ip, config.https_port
        );
    }

    println!("\nExternal Access (requires port forwarding):");
    println!(
        "  • HTTP:  http://{}:{}  (if port {} forwarded)",
        public_ip, config.http_port, config.http_port
    );
    if config.has_certificates {
        println!(
            "  • HTTPS: https://{}:{}  (if port {} forwarded)",
            public_ip, config.https_port, config.https_port
        );
    }
...
fn print_ssl_setup_guidance() {
    ...
    println!(
        "  3. Copy files: cp /etc/letsencrypt/live/yourdomain.com/{{fullchain,privkey}}.pem ."
    );
